...
def load_data_from_sheets():
    try:
        ...
        hoja = cliente.open_by_key(planilla_id).sheet1
        ...
        with st.form("nuevo_evento_form", clear_on_submit=True):
            ...
            if submitted:
                if not f_actividad or not f_ciudad:
                    st.error("Completa 'Actividad' y 'Ciudad'.")
                else:
                    ...
                    if guardar_todo_en_sheets(df_actualizado):
                        st.session_state.agenda = load_data_from_sheets()
                        st.success("¡Actividad registrada de forma permanente en Google Sheets!")
                        st.rerun()

# ------------------------------------------
# TAB 3: MODIFICAR / ELIMINAR REGISTROS
# ------------------------------------------
            ...
            if actividad_seleccionada:
                ...
                with st.form("form_edicion"):
                    ...
                    if boton_eliminar:
                        ...
                        if guardar_todo_en_sheets(df_sheet):
                            st.session_state.agenda = load_data_from_sheets()
                            st.warning("¡Registro eliminado de la planilla!")
                            st.rerun()
